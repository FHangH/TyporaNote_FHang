...
- 功能描述：将`C++`代码，以`C语`言的`形式`进行`编译`
- 作用：用于`C/C++混合开发`时，`C++`中`使用C`写的`第三方库`时使用
- 注意：`C`不支持`函数重载`
...
- 代码示例：
  ...
  以上是 `extern C` 的使用方式，但这个程序`编译会报错`；

  在 `C/C++` 中，编译时转汇编，依据`编译器`的`区别`，函数名会变`改名`：
  ...
  所以，当`C++`文件中，使用`extern C`修饰重载函数后，编译会`出错`
...
- `demo.cpp`
  ...
  在`C++`文件中，不能直接使用`C`文件中的函数，因为`编译器`的`区别`，`相同`的`函数名`，在编译时，函数的名称会`变得不同`，所以`C++`文件中就找不到`C`中的`函数声明和实现`，只能`指向`自己文件中的声明；

  所以需要 `extern C`修饰，这样编译器就会在编译时修改函数名，`修改成C的规范`，这样`C++`文件中就可以找到`C`文件中的函数声明和实现;

- 补充：如果别的 `C` 文件中要使用 `math.h` ，直接包含 `math.h` 会`报错`，因为 `C `的`编译器` 无法`识别 extern C`



- 注意：
  - `extern C` 直接修饰 `C的头文件`内的`声明`即可，`同时`修饰`实现`也可以，但`没必要`；唯独`不能`只修饰`实现`；
...
- 补充知识：
  1. Cpp 文件开头默认会有`#define __cpluscplus`，用于编译时标识自己是 Cpp 文件
  2. Cpp 头文件中一般可以使用： 
     - `#ifdef 项目文件名`
     - `#endif`
     - 包含头文件内容
...
- 代码示例：
  ...
  当 `Cpp` 文件包含这个头文件时，编译时会识别到 `#ifdef`和`#endif`，这样 `extern C`就可以生效；

  当 `C` 文件包含这个头文件时，编译时就会忽略 `extern C`；

  这样的写法格式的好处：`C` 文件 和 `Cpp` 文件 都可以直接包含 `math.h`文件，进行使用
...
-  `#ifndef __文件名_H` 、`#define __文件名_H` 、`#endif`受到C/C++标准支持，不受编译器限制
